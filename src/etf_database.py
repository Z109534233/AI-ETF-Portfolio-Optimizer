...
# Where scripts/refresh_etf_universe.py writes its normalized bulk snapshots
# (Taiwan/US from real authoritative sources, UK curated -- see that
# script's module docstring). This module only ever READS these files; it
# never fetches anything over the network itself, so importing it (i.e.
# every page load) never makes an HTTP request.
_SNAPSHOT_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "etf_universe")
_REGION_BY_COUNTRY = {"United States": "North America", "Taiwan": "Asia Pacific", "United Kingdom": "Europe"}
...
class ETFRecord:
    ...
    # -- Fields added by the Global ETF Universe + Benchmark Architecture
    # round (additive only, same guarantee as the block above) --
    isin: Optional[str] = None                            # ISO 6166 ISIN, where the source discloses it
    fund_group_id: Optional[str] = None                     # links multiple trading lines of the SAME
                                                              # underlying fund/share-class (e.g. a GBP-
                                                              # distributing line and a USD-accumulating line of
                                                              # the same LSE UCITS fund) -- None if not applicable
                                                              # or not yet identified
    source: Optional[str] = None                              # which authoritative source this record came from,
                                                                # e.g. "TWSE ISIN Registry", "Nasdaq Trader Symbol
                                                                # Directory", or "Curated" for the hand-verified
                                                                # records in _BUILTIN_RECORDS below
    source_updated_at: Optional[str] = None                     # ISO date the snapshot this record came from was
                                                                 # last refreshed (see scripts/refresh_etf_universe.py)
...
_BUILTIN_RECORDS: List[ETFRecord] = [
    ...
    # exchange="TPEx" on all 8 below: cross-checked against the real TWSE/
    # TPEx ISIN registry data fetched this round (Global ETF Universe +
    # Benchmark Architecture round) -- these were previously mislabeled
    # "TWSE" (the _tw_etf() default), which also meant they were assigned
    # the wrong Yahoo Finance suffix (".TW" instead of the correct ".TWO");
    # confirmed via data/etf_universe/taiwan.json, generated by
    # scripts/refresh_etf_universe.py straight from TWSE's own registry.
    _tw_etf("00679B", "Yuanta US Treasury 20+ Year ETF", "元大美債20年", "Fixed Income", "Bond",
            ...
            underlying_market="United States", investment_style="Income", exchange="TPEx"),
    ...
    # The 5 originally hand-typed UK records that used to live here (VUSA,
    # VUAG, EQQQ, CSPX, FUSD) are now sourced from the curated
    # data/etf_universe/united_kingdom.json snapshot below instead --
    # loaded via _load_snapshot_records() with identical ticker/name/exchange
    # data, so nothing is lost, just consolidated into the one UK source
    # (scripts/refresh_etf_universe.py's _UK_CURATED list) instead of
    # maintaining two separate hand-written UK lists.
]
...
# ── Bulk universe snapshot loader ────────────────────────────────────────────
# Loads the normalized JSON snapshots that scripts/refresh_etf_universe.py
# writes from real authoritative sources (TWSE/TPEx ISIN registries, Nasdaq
# Trader Symbol Directory) plus the curated UK list -- see that script's
# module docstring for exactly what each source is and why. This module
# never fetches anything itself; a missing/not-yet-generated snapshot file
# just means zero bulk records for that market (curated records above are
# completely unaffected), so the app never breaks if the snapshot job
# hasn't been (re)run.
def _load_snapshot(filename: str) -> List[dict]:
...
def _snapshot_row_to_record(row: dict) -> ETFRecord:
    """One row of a bulk snapshot -> ETFRecord. `sector`/`benchmark`/
    `investment_style` are required ETFRecord fields that bulk sources
    don't disclose per-security (unlike the individually hand-curated
    records above, which do have real sector/benchmark values) -- these
    get an explicit "not individually curated" placeholder rather than a
    guessed value, so the UI/report can tell curated from bulk-imported at
    a glance via the `source` field."""
    country = row.get("listing_market", "")
    asset_class = row.get("asset_class") or "Equity"
    return ETFRecord(
        ticker=row["ticker"], name=row.get("name") or row["ticker"],
        region=_REGION_BY_COUNTRY.get(country, "Other"), country=country,
        currency=row.get("currency") or "USD", exchange=row.get("exchange") or "",
        category=asset_class, sector=asset_class,
        benchmark="N/A (bulk-imported, not individually curated)",
        asset_type=row.get("asset_type") or "Equity ETF", investment_style="Blend",
        yahoo_symbol=row.get("provider_symbol") or row["ticker"],
        issuer=row.get("issuer"), listing_date=row.get("listing_date"),
        underlying_market=row.get("underlying_market"),
        management_style=row.get("management_style"),
        return_type=row.get("return_type") or "Standard",
        yahoo_status=row.get("provider_status") or "unknown",
        isin=row.get("isin"), fund_group_id=row.get("fund_group_id"),
        source=row.get("source"), source_updated_at=row.get("source_updated_at"),
    )


def _load_snapshot_records(filename: str) -> List[ETFRecord]:
    return [_snapshot_row_to_record(row) for row in _load_snapshot(filename)]


def _merge_by_priority(*tiers: List[ETFRecord]) -> List[ETFRecord]:
    """Merge multiple priority tiers of records (highest priority first).
    A ticker already claimed by an earlier (higher-priority) tier is never
    added again from a later tier -- see the callsite below for why this
    matters: ticker strings are NOT globally unique across real-world
    exchanges (e.g. LSE's "EQQQ" and a completely unrelated Nasdaq-listed
    "EQQQ" both genuinely exist), and this flat, ticker-keyed registry can
    only hold one record per ticker string. Individually-verified/curated
    data always wins over a raw bulk import, since a raw bulk import can't
    know it's colliding with a *different, real* security under the same
...
# Priority order (highest first): hand-curated US/Taiwan records, then the
# curated (individually-verified, not bulk-scraped) UK snapshot, then the
# real bulk imports (Taiwan TWSE/TPEx, US Nasdaq Trader) -- see this
# module's + scripts/refresh_etf_universe.py's docstrings. A handful of US
# bulk-imported tickers (e.g. "EQQQ", "FUSD") are consequently NOT present
# under those exact symbols, because a curated UK record with the identical
# ticker string already claims it; this is a disclosed, deliberate
# trade-off of a ticker-keyed registry, not data loss of a market's own
# universe (Taiwan/UK bulk-vs-curated collisions do not occur in practice).
_ALL_RECORDS: List[ETFRecord] = _merge_by_priority(
    _BUILTIN_RECORDS,
    _load_snapshot_records("united_kingdom.json"),
    _load_snapshot_records("taiwan.json"),
    _load_snapshot_records("united_states.json"),
)


# ── Module-level singleton + convenience functions ──────────────────────────
ETF_DATABASE = ETFDatabase(_ALL_RECORDS)
...
def validate_etf_database() -> dict:
    ...
    # never find a duplicate that genuinely exists in the source data. Uses
    # _ALL_RECORDS (curated + bulk-imported, pre-dedup-by-ticker) so this
    # also catches any duplicate introduced by a bulk snapshot refresh, not
    # just the hand-curated records.
    raw_records = _ALL_RECORDS
    issues = []
    seen = {}
